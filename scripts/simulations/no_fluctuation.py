'''
This script analyses the proportion of tumours metastasising before earlist possible detection in the absence of
...
for i in range(N_iters):
    ...
    # Get the CA-125 at each of the times - vecotirse for speed - no fluctuations!!!
    df_raw['inter_healthy'] = df_raw['mu'] + (df_raw['dt'] / 10) * df_raw['dt_d'] 
    df_raw['ca125_healthy'] = np.exp(df_raw['inter_healthy'])
...
df_results.prop_met.mean() # 62.9
df_results.prop_met.quantile([0.025, 0.975]) # 62.5 - 63.3

df_results.to_csv('./../../output/simulations/results_no_fluctuation_generated.csv')
