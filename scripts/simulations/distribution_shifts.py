'''
Sensitivity analysis by perturbing distributions of tumour growth rates and primary tumour volumes left and right ....
'''
import pandas as pd
import numpy as np
...
df_base = pd.DataFrame({'ix': ix_vec, 
                       'dt': T_vec})

df_base = pd.merge(df_base, X_healthy, on='ix', how='left')
df_base = pd.merge(df_base, mus, on='ix', how='left')
df_base = pd.merge(df_base, X, on='ix', how='left')


# Get the CA-125 at each of the times - vecotirse for speed
df_base['inter_healthy'] = df_base['mu'] + (df_base['dt'] / 10) * df_base['dt_d'] 
n = len(df_base)
loc = df_base['inter_healthy'].to_numpy()
scale = np.exp(df_base['logsig'].to_numpy())
df_base['ca125_healthy'] = np.exp(loc + scale * np.random.standard_t(df=5, size=n))

df_base['ca125_bl'] = np.exp(df_base['inter_healthy'])
...
for _, perm_row in perm_df.iterrows():
    ...
    print(perm_row)
    # Permute parameters
    for col in perm_row.index:
        df_raw[col] *= perm_row.loc[col]
    ...
    dict_ = {}
    for col in perm_row.index:
        dict_['perm_{}'.format(col)] = perm_row.loc[col]
    
    dict_['prop_met'] = df_det.met.value_counts(normalize=True).loc[1]
    df_results.append(dict_)

df_results = pd.DataFrame(df_results)

df_results.to_csv('./../../output/simulations/results_distribution_shifts_generated.csv')
